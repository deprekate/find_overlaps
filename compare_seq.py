...
parser.add_argument('-n', '--number', type=int, default=None, help='The sge index of this processes')
...
seqs = dict()
for name in files[args.number-1 : args.number]:
	args.outfile.write('\t')
	args.outfile.write(name)
	path = os.path.join(args.directory, name)
	with open(path) as f:
		for line in f:
			seqs[line.rstrip().split('\t')[args.column - 1]] = True
